...
The bot never extracts `Translation` rows on their own; each parent
entity's emit function appends a `{{Translation | type=… | …}}` call
to its own page, right after the entity's structural row. The
`{{Translation}}` MediaWiki template's job is to call `#cargo_store`
on this table.
...
| --- | --- | --- | --- |
| `unit` | [`Unit`](../Unit.md) | `Unit.id` | `emit_unit_page` |
| `unit_ability` | [`UnitAbility`](../UnitAbility.md) | `UnitAbility.ability_id` (composite) | `emit_unit_page` |
| `attack_passive` | [`AttackPassive`](shared/AttackPassive.md) | `AttackPassive.attack_passive_id` | `emit_attack_passive_page` |
| `faction` | [`Faction`](../Faction.md) | `Faction.id` | `emit_faction_page` |
...
emitted as a `{{Translation | …}}` invocation on the parent entity's
