...
reference data the wiki ships pre-populated. `UnitAttackDef` slots
...
```mediawiki
{{#cargo_declare:_table=AttackPassiveDef
...
- **`attack_passive_id`** is the primary key and the value `UnitAttackDef`
...
- **Piercing Strike** (`base_passive_strike_pierce_<rank>_*`) is **not**
  ...
  synthesis path, not in `AttackPassiveDef`.

## Related tables

- [`UnitAttackDef`](../UnitAttack.md) — references via `default_passive`,
...
</noinclude><includeonly>{{#cargo_store:_table=AttackPassiveDef
