...
```mediawiki
{{#cargo_declare:_table=EntryDef
| type = String (allowed values=attack_archetype,movement,creature_type,FactionCityName,resource,hero_stat,unit_stat,law_level,skill_level)
| subtype = String
...
- **Primary key is `(type, subtype, variant)`** as a tuple. `variant`
  is sparse — it's set for sub-row translations (`type='law_level'`,
  `type='skill_level'`) where the same `(type, subtype)` pair needs
  to disambiguate per-level rows, and left unset for everything else.
  Joins from per-unit columns include the type discriminator: e.g.
  `UnitDef.attack_type` joins to `EntryDef.subtype WHERE EntryDef.type='attack_archetype'`.
- **`variant`** carries sub-row indices for rows that fold per-(entity,
  level) data into this table — currently `type='law_level'` and
  `type='skill_level'` (folded back from the now-removed
  `LawLevelTranslationDef` and `SkillLevelTranslationDef`). String-typed
  so it can hold either integer levels (`"1"`, `"2"`, `"3"`) or
  freeform variant codes if future sub-rows need them.
...
directory — the `EntryDef` Cargo table name only surfaces in the table
schema and the `{{EntryDef | …}}` template invocations *inside* each
page. The `EntryDef` abstraction is internal; user-facing wiki paths use
...
`{{FactionDef}}` and `{{FactionTranslation}}` blocks, in numeric source
order). Cargo stores the rows in the unified `EntryDef` table just the
...
table. For example, `AttackPassiveDef` carries `pattern_token` and `rank`
...
- [`UnitDef`](../Unit.md) — references multiple Entry types via
  `attack_type`, `move_type`, `creature_type` columns.
- [`AttackPassiveDef`](AttackPassiveDef.md) — separate table; has columns
...
{{EntryDef | type=<type> | subtype=<subtype> | ...}} which routes through
this template's #cargo_store call.
</noinclude><includeonly>{{#cargo_store:_table=EntryDef
| type={{{type|}}}
| subtype={{{subtype|}}}
