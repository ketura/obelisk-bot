...
metadata, and classification flags. The bot emits one `{{UnitDef | …}}`
...
- [`UnitAbilityDef`](UnitAbilityDef.md) and its splinters — joined on
  `unit_id = id` (1:N).
...
</noinclude><includeonly>{{#cargo_store:_table=UnitDef
| id={{{id|}}}
| unused={{{unused|}}}
| faction={{{faction|}}}
…
