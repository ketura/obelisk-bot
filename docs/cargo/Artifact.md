...
1. `{{Artifact | id=… | name=… | description=… | slot=… | rarity=… | …}}`
...
2. `{{Translation | type=artifact | target_id=<id> | …}}` — name +
   description i18n (per D-026).
3. N × `{{Bonus | parent_type=artifact | parent_id=<id> | …}}` — the
...
- **No sub-fields for the `bonuses[]`** here — they emit as
  separate `Bonus` rows. Query artifacts with their bonuses via
...
- [`Bonus`](shared/Bonus.md) — joined on `id = parent_id` (1:N).
- [`Translation`](shared/Translation.md) — one row per artifact
  with `type=artifact`.
