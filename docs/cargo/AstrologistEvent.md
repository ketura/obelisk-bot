...
1. `{{AstrologistEvent | id=… | category=week|month | name=… | desc=… | …}}`
2. `{{Translation | type=astrologist_event | target_id=<id> | …}}` — name+desc in the 16 supported languages

## Schema

```mediawiki
{{#cargo_declare:_table=AstrologistEvent
| id = String
...
- [`Translation`](shared/Translation.md) — one row per event with `type=astrologist_event`, `target_id=id`.
