...
`FactionLawTierDef` rows are emitted inline on the parent `FactionDef` page alongside the `LawTreePositionDef` rows.

## Schema

```mediawiki
{{#cargo_declare:_table=FactionLawTierDef
...
- [`LawTreePositionDef`](LawTreePositionDef.md) — joined on `(faction, tier)`.
