...
inherits. Individual `HeroDef` rows carry only the *deltas* — fields the
...
template). When a `HeroDef` row omits a class-level field, the wiki
display layer joins to `HeroClassDef` to fill it in. See D-026.
...
treating the `HeroClassDef` row as a starting point.

## Schema

```mediawiki
{{#cargo_declare:_table=HeroClassDef
...
The `HeroDef` table mirrors most of the columns above. When the bot
emits a hero row:

- **Faction heroes (108):** the bot omits all 11 class-level columns
  from the `HeroDef` row — they're all defaulted from `HeroClassDef`.
  Display layer reads the field from `HeroDef`; if absent, joins
  `HeroClassDef` on `Hero.class_id = HeroClass.id` and reads from
  there.
...
- [`HeroDef`](HeroDef.md) — joined on `Hero.class_id = HeroClass.id` (1:N).
...
- [`HeroSubClassDef`](HeroSubClassDef.md) — the named prestige classes
