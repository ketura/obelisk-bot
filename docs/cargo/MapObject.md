...
1. `{{MapObject | id=… | category=… | name=… | desc=… | …}}`
2. `{{Translation | type=map_object | target_id=<id> | <lang>_name=… | <lang>_desc=… | …}}`
...
- **`cities/`** — already modeled in the [`Building`](Building.md) table.
- **`items/`** — these are placement metadata for artifacts on the map; the canonical Artifact data is in the [`Artifact`](Artifact.md) table.
...
- [`Translation`](shared/Translation.md) — one row per MapObject with `type='map_object'`, `target_id=id`.
- [`Building`](Building.md) — sibling table covering city-buildings (`DB/objects_logic/cities/`).
- [`Artifact`](Artifact.md) — canonical artifact data; map placements in `DB/objects_logic/items/` reference these by id.
- [`Unit`](Unit.md) — `guard_units` entries reference Unit ids.
