...
1. `{{SpellDef | id=… | name=… | school=… | rank=… | …}}` — main row
   with the split learn-cost columns inline.
2. 4 × `{{SpellRankDef | spell_id=<id> | level=<1-4> | …}}` paired with
   4 × `{{SpellRankTranslationDef | spell_id=<id> | level=<1-4> | …}}`
...
- [`SpellRankDef`](SpellRankDef.md) — joined on `id = spell_id` (1:N, 4
  rows per spell).
- [`SpellRankTranslationDef`](SpellRankTranslationDef.md) — joined on
