...
the `{{HeroDef}}` and `{{TranslationDef}}` blocks.

## Schema

```mediawiki
{{#cargo_declare:_table=HeroStartSquadDef
...
- [`HeroDef`](HeroDef.md) — joined on `hero_id = id` (N:1).
- [`UnitDef`](UnitDef.md) — joined on `unit_sid = id` (N:1).
