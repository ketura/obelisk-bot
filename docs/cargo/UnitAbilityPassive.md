...
(immunities, disablers, stat overrides), already rolled up to the `Unit`
...
```mediawiki
{{#cargo_declare:_table=UnitAbilityPassive
...
- **`sequence_effect`** marks passives that interact with the
  ...
  `ability_id`. That's intentional — the parent `UnitAbility` row
  carries everything else (name, desc, sids).
- **Passives' classification data** (immunities, disablers, creature
  type) lives on the `Unit` row, not here. Those are properties of the
