...
Because of those, this table doesn't carry a companion `{{TranslationDef}}` row — the source data ships in English only.

## Page layout

`Data:Difficulty/<id>` carries a single `{{DifficultyDef}}` row.

## Schema

```mediawiki
{{#cargo_declare:_table=DifficultyDef
