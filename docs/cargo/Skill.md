...
A skill carries identity (id, variant, type), shared name + desc SIDs, and a count of mastery levels. Per-level data (overrides, offered sub-skills, bonuses) lives on [`SkillLevel`](SkillLevel.md). The sub-skills available from a skill live on [`SubSkill`](SubSkill.md), recovered by scanning every level's `offered_sub_skills` list.

## Page layout

`Data:Skill/<id>` is self-contained — it carries the skill's own row plus every sub-skill referenced by any of its levels:

1. `{{Skill | id=… | variant=… | skill_type=… | …}}`
2. `{{Translation | type=skill | target_id=<id> | …}}` — name + desc shared across all levels
3. N × `{{SkillLevel | skill_id=<id> | level=N | …}}`
4. N × `{{SkillLevelTranslation | skill_id=<id> | level=N | … }}` — only when level overrides a name/desc
5. M × `{{Bonus | parent_type=skill_level | parent_id=<id>_L<level> | … }}`
6. K × `{{SubSkill | id=… | parent_skill_id=<id> | …}}` — every sub-skill referenced by any level
7. K × `{{Translation | type=sub_skill | target_id=<sub_skill_id> | …}}`
8. K' × `{{Bonus | parent_type=sub_skill | parent_id=<sub_skill_id> | …}}`
...
- **`skill_type`** is one of `Common` / `Class` / `Faction`. Pseudo skills (the 12 `skill_pseudo_*` entries) carry no skill_type — they're internal mechanism skills not shown in the hero UI.
...
- [`SkillLevel`](SkillLevel.md) — joined on `id = skill_id` (1:N where N=1..3).
- [`SubSkill`](SubSkill.md) — joined on `id = parent_skill_id`. Recovered by scanning every level's `offered_sub_skills`.
- [`Bonus`](shared/Bonus.md) — skill-level bonuses join on `parent_type='skill_level' AND parent_id LIKE id || '_L%'`.
- [`Translation`](shared/Translation.md) — one row per skill with `type=skill`, `target_id=id`.
