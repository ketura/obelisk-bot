...
each slot references a shared [`AttackPassiveDef`](shared/AttackPassive.md)
...
```mediawiki
{{#cargo_declare:_table=UnitAttackDef
...
- **`<slot>_passive`** references [`AttackPassiveDef`](shared/AttackPassive.md)
...
- [`AttackPassiveDef`](shared/AttackPassive.md) — joined on
  `<slot>_passive` (4 references per row).
- [`UnitDef`](UnitDef.md) — joined on `unit_id = id` (1:1).
...
</noinclude><includeonly>{{#cargo_store:_table=UnitAttackDef
| unit_id={{{unit_id|}}}
