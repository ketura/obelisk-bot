...
Each level carries the law-points cost to enact (separate from the tier-unlock threshold), and the resolved description for *that level's* parameter substitutions. Bonuses awarded at this level are emitted as `{{Bonus | parent_type=law_level | parent_id=<law_id>_L<level> | …}}` rows.

## Schema

```mediawiki
{{#cargo_declare:_table=LawLevel
| law_id = String
...
- **No `name` column** — the law's name is shared across all levels and lives on `Law.name` / `Translation`.

## Related tables

- [`Law`](Law.md) — joined on `law_id = Law.id` (N:1).
- [`Bonus`](shared/Bonus.md) — joined on `parent_type='law_level' AND parent_id = law_id || '_L' || level`.
- [`LawLevelTranslation`](LawLevelTranslation.md) — joined on `(law_id, level)`. Carries per-level descriptions in 15 non-English languages.
