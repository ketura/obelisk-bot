...
The bot emits one `{{FactionDef | …}}` template invocation at the top of
...
as `EntryDef` rows of `type=FactionCityName` (one row per city, see
...
```mediawiki
{{#cargo_declare:_table=FactionDef
...
- **`id`** is the primary key. Six fixed values; `neutral` is *not* a
  faction here even though `FactionDef` enum includes it for unit
...
- [`EntryDef`](shared/Entry.md) where `type='FactionCityName'` — joined
  via `subtype LIKE '<faction>_%'` to surface this faction's
  randomization-pool city names.
- [`UnitDef`](UnitDef.md) — joined via `Unit.faction = Faction.id` (1:N).
