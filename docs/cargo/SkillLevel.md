...
- [`Skill`](Skill.md) — joined on `skill_id = Skill.id` (N:1).
- [`SubSkill`](SubSkill.md) — joined on `offered_sub_skills HOLDS SubSkill.id`.
- [`Bonus`](shared/Bonus.md) — joined on `parent_type='skill_level' AND parent_id = skill_id || '_L' || level`.
- [`SkillLevelTranslation`](SkillLevelTranslation.md) — joined on `(skill_id, level)`. Carries per-level name/desc overrides in 16 languages, populated only when the level has its own `name_sid` / `desc_sid`.
