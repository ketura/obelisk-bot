...
1. `{{Building | id=<faction>_<sid>_L<level> | faction=… | sid=… | level=… | name=… | desc=… | …}}`
2. `{{Translation | type=building | target_id=<id> | <lang>_name=… | <lang>_desc=… | …}}`
...
- **`name` / `desc`** carry English defaults inline; per-language values live on the matching `{{Translation | type=building}}` row.
...
- **`bonusesPerLevel`** — per-level passive bonuses on the building (e.g. mains' `+500 gold daily`). The data exists in source but isn't currently emitted; would extend the unified `Bonus` table with `parent_type='building_level'` if added later. Out of scope for this pass.
...
- [`Translation`](shared/Translation.md) — one row per Building with `type='building'`, `target_id=id`, carrying name + desc in 16 languages.
- [`Unit`](Unit.md) — for `category='hires'` rows, `units_hire_sid` joins to `Unit.id` (the base unit; upgrade variants chain via Unit's own self-joins).
- [`Building`](Building.md) (self) — `prereqs HOLDS '<sid>_L<level>'` finds buildings unlocked by a given prereq tier.
