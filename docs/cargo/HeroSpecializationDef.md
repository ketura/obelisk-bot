...
```mediawiki
{{#cargo_declare:_table=HeroSpecializationDef
| id = String
| name = String
| desc = Wikitext
| name_sid = String
...
- **`name` / `desc`** carry the resolved English text inline.
  Translations live on the `{{TranslationDef | type=hero_specialization}}`
  row.
...
- [`HeroDef`](HeroDef.md) — joined on `id = Hero.specialization_id` (1:1
...
- [`HeroSpecializationBonus`](HeroSpecializationBonus.md) — joined
  on `id = spec_id` (1:N).
- [`TranslationDef`](shared/Translation.md) — one row per spec with
