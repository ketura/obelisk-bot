...
[`HeroClass`](HeroClass.md), and any class-default fields the hero
...
1. `{{Hero | id=… | name=… | motto=… | desc=… | start_skills=… | start_magics=… | …}}`
...
3. `{{Translation | type=hero_motto | target_id=<id> | …}}` —
   ...
   path; per D-027).
4. `{{HeroStartSquad | …}}` × N — one per starting squad slot (primary
   + alt collapsed via `variant` discriminator).

## Schema

```mediawiki
{{#cargo_declare:_table=Hero
| id = String
| name = String
| motto = Wikitext
| desc = Wikitext
...
- **`name`/`motto`/`desc`** carry the resolved English text inline.
  Translations live on the two `Translation` rows (one for name+desc,
  one for motto).
...
- **`faction`/`class_type`** are denormalized from the class so
  ...
  join.
- **`specialization_id`** points at a row in `HeroSpecialization`
  (one specialization per hero, 1:1 ratio). Schema for that table
  comes in a follow-up.

## Related tables

- [`HeroClass`](HeroClass.md) — joined on `class_id = id` (N:1).
  Carries the class defaults the Hero row may override.
- [`HeroStartSquad`](HeroStartSquad.md) — joined on `hero_id = id`
  (1:N). Each row is one slot in primary or alt starting army.
- `Skill` (future) — joined on `start_skills` LIKE `%<sid>%` or via
  Cargo's `HOLDS` operator on the List column.
- `Spell` (future) — same join pattern via `start_magics`.
- [`HeroSpecialization`](HeroSpecialization.md) — joined on
  `specialization_id = id` (N:1). The hero's unique passive.
  *(Schema lands in a follow-up.)*
- [`Translation`](shared/Translation.md) — two rows per hero
