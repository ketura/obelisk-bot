...
1. `{{ItemSet | id=… | name=… | items_in_set=… | …}}` — main row.
2. `{{Translation | type=item_set | target_id=<id> | …}}` — name
   translations (per D-026).
3. For each tier: a `{{ItemSetTier | id=… | required_amount=… | …}}`
   row, a `{{Translation | type=item_set_tier | target_id=<tier_id>
   | <lang>_desc=… }}` row (desc-only, no name), and N
   `{{Bonus | parent_type=item_set_tier | parent_id=<tier_id> | …}}`
   rows.

## Schema

```mediawiki
{{#cargo_declare:_table=ItemSet
| id = String
...
- **No description on the set itself** — descriptions are per-tier
  and live on `ItemSetTier`.

## Related tables

- [`ItemSetTier`](ItemSetTier.md) — joined on `id = set_id` (1:N,
  1-3 tiers per set).
- [`Artifact`](Artifact.md) — `items_in_set` references
  `Artifact.id`; `Artifact.artifact_set_id` is the inverse pointer.
- [`Translation`](shared/Translation.md) — one row per set with
