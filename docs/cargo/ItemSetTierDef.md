...
One unlock tier of an `ItemSetDef`. The hero unlocks the tier's bonuses
...
```mediawiki
{{#cargo_declare:_table=ItemSetTierDef
...
- **`description`** carries the resolved English text inline.
  Translations live on a `{{TranslationDef | type=item_set_tier |
  target_id=<id>}}` row (desc-only — tiers have no name).

## Related tables

- [`ItemSetDef`](ItemSetDef.md) — joined on `set_id = id` (N:1).
- [`BonusDef`](shared/Bonus.md) — joined on `id = parent_id` (1:N) where
  `Bonus.parent_type = 'item_set_tier'`. Each tier has 1-6 bonus
  effects.
- [`TranslationDef`](shared/Translation.md) — one row per tier with
